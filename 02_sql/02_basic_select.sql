...
select emp_id as 직원ID, 
       emp_name as "직원 이름", -- 별칭 공백이 들어가면 "" 로 묶어준다
       hire_date as 입사일, 
       salary as 급여, 
       dept_name as 소속부서
from   emp;
...
-- where comm_pct >= 0.2 and comm_ pct <= 0.3;
...
-- where emp_name like '%en' -- en으로 끝나는 name
 where emp_name like '%eve%'; -- eve를 포함한 name
...
select emp_id, emp_name
from emp
where emp_name like '%$%%' escape '$'; -- escape '문자' 문자%, 문자_% 와 _ 그대로 비교 // $% -> % 


-- EMP 테이블에서 부서명(dept_name)이 null인 직원의 ID(emp_id), 이름(emp_name), 부서명(dept_name)을 조회.
select emp_id, emp_name, dept_name
from emp
where dept_name is null; -- // 컬럼 = null 비교 불가
