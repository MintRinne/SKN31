...
-- 날짜/시간의 차이 계산
select datediff(curdate(), '2024-10-10'); -- 2025-10-20 - 2025-10-10  -- 오늘 - 2024-10-10 며칠 지났는지
...
select salary,
       case when salary >= 10000 then '1등급' 
            else '2등급' end "급여 등급" -- when salary between 10000 and 19999
from emp;
...
FROM
    emp
WHERE
    job IN ('AD_PRES' , 'FI_ACCOUNT', 'PU_CLERK', 'IT_PROG');
